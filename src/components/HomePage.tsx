...
import { Language, Student, Notice, SabaqDiaryEntry, Teacher, HeroSlide, GalleryItem, BlogPost } from '../types';
...
export const HomePage: React.FC<HomePageProps> = ({
  ...
  return (
    <div className="space-y-12">
      ...
      {/* 7. Latest Blog Articles & Guidance */}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-2 border-b border-slate-200">
            <div>
              <div className="flex items-center gap-2 text-emerald-800">
                ...
                <h2 className="text-lg sm:text-xl font-bold text-slate-900">
                  {lang === 'bn' ? 'ইসলামিক প্রবন্ধ, হিফজ নির্দেশিকা ও ব্লগ' : 'Latest Articles & Academy Blog'}
                </h2>
              </div>
              <p className="text-xs text-slate-500 mt-0.5">
                {lang === 'bn'
                  ? 'কুরআন হিফজের কৌশল, সহীহ তাজবীদ ও সন্তান প্রতিপালনে উস্তাদদের মূল্যবান প্রবন্ধসমূহ'
                  : 'Practical guides on Quran memorization, Tajweed, and Islamic parenting by our Ustads'}
              </p>
            </div>

            <button
              onClick={() => onNavigate('blog')}
              className="text-xs font-bold text-emerald-800 hover:text-emerald-950 flex items-center gap-1 transition self-start sm:self-auto cursor-pointer"
            >
              <span>{lang === 'bn' ? 'সকল প্রবন্ধ দেখুন' : 'View All Articles'}</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {blogs.slice(0, 3).map((post) => (
              <div
                ...
                className="group bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-2xs hover:shadow-lg hover:border-emerald-400 transition-all duration-300 cursor-pointer flex flex-col justify-between"
              >
                <div>
                  <div className="relative aspect-[16/10] w-full overflow-hidden bg-slate-100">
                    ...
                    <div className="absolute top-2 left-2">
                      <span className="px-2 py-0.5 rounded-md bg-emerald-950/80 backdrop-blur-sm text-amber-300 text-[10px] font-bold">
                        {lang === 'bn' ? post.categoryLabel : post.categoryLabelEn}
                      </span>
                    </div>
                    <div className="absolute bottom-2 right-2 px-1.5 py-0.5 rounded bg-black/70 backdrop-blur-sm text-[10px] font-mono text-slate-200 flex items-center gap-1">
                ...
                <div className="p-4 pt-0 flex items-center justify-between text-[11px] text-slate-500 border-t border-slate-50 mt-2">
                  <span className="font-semibold text-slate-700 truncate max-w-[140px]">
                    {post.author}
                  </span>
                  <span className="text-emerald-700 font-bold group-hover:translate-x-0.5 transition flex items-center gap-0.5">
